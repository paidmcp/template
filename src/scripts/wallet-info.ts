...
const baseAccount = await new WalletManagerEvm(config.SEED_PHRASE, { provider: config.BASE_RPC_URL }).getAccount();
const plasmaAccount = await new WalletManagerEvm(config.SEED_PHRASE, { provider: config.PLASMA_RPC_URL }).getAccount();
